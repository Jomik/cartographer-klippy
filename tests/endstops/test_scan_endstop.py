...
from cartographer.endstops.scan_endstop import Probe, ScanEndstop
from cartographer.printer_interface import HomingState, Mcu
...
@pytest.fixture
def endstop(mcu: Mcu[object, Sample], probe: Probe) -> ScanEndstop[object, Sample]:
    return ScanEndstop(mcu, probe)


@pytest.fixture
def homing_state(mocker: MockerFixture, endstop: ScanEndstop[object, Sample]) -> HomingState:
    mock = mocker.Mock(spec=HomingState, autospec=True)
    mock.endstops = [endstop]
    return mock


def test_do_nothing_when_not_homing(
    mocker: MockerFixture, endstop: ScanEndstop[object, Sample], homing_state: HomingState
):
...
def test_endstop_is_triggered(endstop: ScanEndstop[object, Sample], probe: Probe):
    probe.measure_distance = lambda time=0: 1

    assert endstop.query_is_triggered(0) is True


def test_endstop_is_not_triggered(endstop: ScanEndstop[object, Sample], probe: Probe):
    probe.measure_distance = lambda time=0: 1
